...
CSV 的 `email`、`cc`、`bcc` 欄位，若收件人是台大信箱，可以只填學號（例如 `B12345678`），系統會自動補上 `@ntu.edu.tw`，不論你用台大信箱還是 Gmail 寄送都適用（這個規則看的是「收件人」，跟寄件帳號無關）。已包含 `@` 的地址（例如收件人本來就是 Gmail 或其他信箱）則維持原樣，不會被更動。
